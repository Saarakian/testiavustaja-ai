...
const getToolDefinitions = (lang: Language): FunctionDeclaration[] => {
  ...
  return [
    {
      name: 'updateReport',
      description: isFi ? 'Päivittää testiraportin tekstisisältöä.' : 'Updates text content of the test report.',
      parameters: {
        type: Type.OBJECT,
        properties: {
          section: {
          ...
          content: {
            type: Type.STRING,
            description: isFi 
              ? "Sisältö. Tämän on oltava asiatyylistä, huoliteltua kirjakieltä." 
              : "Content. Must be formal, academic written language.",
          },
        },
...
const App: React.FC = () => {
  ...
  const getSystemInstruction = (lang: Language, plan: string | null, currentReport: TestReport) => {
    ...
    let base = isFi 
      ? `ROOLI: Olet asiantunteva testausavustaja. TÄRKEIN TEHTÄVÄ: Muunna käyttäjän puhekieli (slangi, finglish) raporttiin huolitelluksi kirjakieleksi (asiatyyli). PUHE: Puhu selkeää suomea. Raportin otsikon ('title') on oltava muodossa "Testiraportti: [Aihe]".`
      : `ROLE: You are an expert testing assistant. MAIN TASK: Convert user's spoken language (informal, slang) into FORMAL ACADEMIC WRITTEN ENGLISH for the report. SPEECH: Speak clear, professional English. The report title must be formatted as "Test Report: [Subject]".`;

    // --- AUTOMATION INSTRUCTIONS ---
    base += isFi
      ? `\n\nAUTOMAATTINEN SISÄLLÖNTUOTANTO (TÄRKEÄ):
         1. JOHDANTO: Kirjoita 'introduction' (johdanto) ITSENÄISESTI heti kun tiedät testin aiheen tai näet suunnitelman. Älä odota käyttäjän sanelua. Kuvaile tausta ja oletettu tarkoitus.
         2. JOHTOPÄÄTÖKSET: Kun käyttäjä raportoi tuloksia ('results'), analysoi niitä jatkuvasti ja päivitä 'conclusion' (johtopäätökset) -osiota itsenäisesti. Sinun tulee päätellä, toimiko testi vai ei.
         3. TIIVISTELMÄ: Kun raportissa on tavoitteet ja tuloksia, kirjoita 'abstract' (tiivistelmä) oma-aloitteisesti.`
      : `\n\nAUTOMATED CONTENT GENERATION (IMPORTANT):
         1. INTRODUCTION: Write the 'introduction' INDEPENDENTLY immediately once you know the test topic or see the plan. Do not wait for dictation. Describe background and purpose.
         2. CONCLUSION: As the user reports 'results', continuously analyze them and update the 'conclusion' section independently. Deduce whether the test passed or failed.
         3. ABSTRACT: When the report has objectives and results, write the 'abstract' proactively.`;
    // --------------------------------

    base += isFi
      ? `\nKAMERA: Kun saat 'SNAPSHOT_TAKEN', kysy mihin osioon kuva liitetään ja käytä 'addImageToReport'.`
      : `\nCAMERA: When you receive 'SNAPSHOT_TAKEN', ask where to attach the image and use 'addImageToReport'.`;

    base += `\nPROTOCOL: Use 'updateReport' immediately when new info is available. Don't wait.`;

    if (plan) {
      base += isFi
        ? `\nTESTISUUNNITELMA (Analysoi heti ja täytä pohjatiedot):\n"""${plan}"""`
        : `\nTEST PLAN (Analyze immediately and pre-fill report):\n"""${plan}"""`;
    } else {
      base += isFi
        ? `\nEI SUUNNITELMAA: Haastattele käyttäjää ja täytä raporttia vastauksien perusteella.`
        : `\nNO PLAN: Interview the user and fill the report based on answers.`;
    }
  ...
  const updateReportState = useCallback((section: string, content: string) => {
    setReport((prev) => {
      ...
      } else if (['methodology', 'testMaterial', 'theoreticalFramework', 'discussion', 'introduction', 'abstract', 'conclusion'].includes(section)) {
         // Allow overwriting/appending for intro, abstract, conclusion to allow AI refinement
         const key = section as 'methodology' | 'testMaterial' | 'theoreticalFramework' | 'discussion' | 'introduction' | 'abstract' | 'conclusion';
         // Check if content seems to be a complete rewrite or an addition. 
         // For automated sections like abstract/intro, we often want to refine (replace) or append intelligently.
         // Simple heuristic: If it's a new thought, append. If it looks like a full paragraph rewrite, replace. 
         // For simplicity in this app, we will append with a space, but user can always request rewrite verbally.
         newReport[key] = prev[key] ? `${prev[key]} ${content}` : content;
      } else {
          ...
          if (blob) {
             ...
             sessionRef.current.then((session: any) => {
                 if (isConnectedRef.current) {
                     session.send({
                        clientContent: {
                          turns: [{ role: 'user', parts: [{ text: "SNAPSHOT_TAKEN" }] }],
                          turnComplete: true
                        }
                     });
                 }
  ...
  const stopSession = useCallback(async () => {
    ...
    if (sessionRef.current) {
       try { const s = await sessionRef.current; if(s && s.close) s.close(); } catch(e) {}
       sessionRef.current = null;
    }
    if (processorRef.current) { processorRef.current.disconnect(); processorRef.current = null; }
    if (inputSourceRef.current) { inputSourceRef.current.disconnect(); inputSourceRef.current = null; }
    if (inputAudioContextRef.current?.state !== 'closed') { try { await inputAudioContextRef.current?.close(); } catch(e) {} }
    if (outputAudioContextRef.current?.state !== 'closed') { try { await outputAudioContextRef.current?.close(); } catch(e) {} }
  ...
  const startSession = async () => {
    ...
    try {
      ...
      // Using the advanced multimodal live model
      const config = {
          ...
          onopen: () => {
            ...
            scriptProcessor.onaudioprocess = (e: AudioProcessingEvent) => {
              ...
              if (rms < NOISE_THRESHOLD && !isMutedRef.current) {
                 ...
                 if (!isMutedRef.current) setUserVolume(rms * 5); 
              }

              sessionPromise.then((session) => {
                 try { if (isConnectedRef.current) session.sendRealtimeInput({ media: pcmBlob }); } catch (e) {}
              }).catch(() => {});
            };
          ...
          onmessage: async (message: LiveServerMessage) => {
             ...
             try {
               ...
               if (base64Audio) {
                  ...
                  setAiVolume(Math.random() * 0.5 + 0.3);
        ...
        config: {
          ...
          speechConfig: {
            voiceConfig: {
              prebuiltVoiceConfig: { voiceName: 'Kore' } // Kore works well for both languages
            }
          },
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full max-w-lg flex flex-col items-center gap-10 z-0">
          <div className="relative w-[300px] h-[300px] md:w-[350px] md:h-[350px] flex items-center justify-center">
             ...
             <div className="absolute inset-0 z-20 pointer-events-none">
                <Visualizer isActive={status === ConnectionStatus.CONNECTED} isAiSpeaking={isAiSpeaking} isVideoMode={isCameraOn} userVolume={userVolume} aiVolume={aiVolume} />
             </div>
      ...
      <div className="w-full md:w-[500px] bg-slate-900 border-l border-slate-800 flex flex-col h-[40vh] md:h-screen">
         ...
         <div className="flex-1 overflow-y-auto p-6 scrollbar-thin space-y-8">
            <div className="space-y-4">
               ...
               <div>
                 ...
                 <div className="bg-slate-800/50 p-3 rounded-lg text-slate-300 text-xs leading-relaxed italic border border-slate-800"><HtmlContent html={report.abstract || t.sections.noData} /></div>
               </div>
            </div>
            ...
            <div className="space-y-4">
               ...
               <div><h3 className="text-sm font-semibold text-teal-400 mb-1">{t.sections.intro}</h3><div className="text-sm text-slate-300 leading-relaxed"><HtmlContent html={report.introduction || t.sections.noData} /></div></div>
               <div><h3 className="text-sm font-semibold text-teal-400 mb-1">{t.sections.objectives}</h3><div className="text-sm text-slate-300 leading-relaxed bg-teal-900/10 p-2 rounded border-l-2 border-teal-500"><HtmlContent html={report.testObjectives || t.sections.noData} /></div></div>
               {report.theoreticalFramework && (<div><h3 className="text-sm font-semibold text-teal-400 mb-1">{t.sections.theory}</h3><div className="text-sm text-slate-300 leading-relaxed"><HtmlContent html={report.theoreticalFramework} /></div></div>)}
            </div>
            ...
            <div className="space-y-4">
               ...
               {report.testMaterial && (<div><h3 className="text-sm font-semibold text-indigo-400 mb-1">{t.sections.material}</h3><div className="text-sm text-slate-300 leading-relaxed"><HtmlContent html={report.testMaterial} /></div></div>)}
               ...
               <div>
                  ...
                  <div className="space-y-2">
                    {report.results.length === 0 ? <p className="text-sm text-slate-500 italic">{t.sections.noData}</p> : 
                      report.results.map((res, i) => <div key={i} className="bg-slate-800 p-2 rounded text-sm text-slate-200 border-l-2 border-emerald-500"><HtmlContent html={res} /></div>)
                    }
                  </div>
               </div>
               {report.discussion && (<div className="mt-4"><h3 className="text-sm font-semibold text-emerald-400 mb-1">{t.sections.discussion}</h3><div className="text-sm text-slate-300 leading-relaxed bg-emerald-900/10 p-3 rounded"><HtmlContent html={report.discussion} /></div></div>)}
            </div>
            ...
            <div className="space-y-4 pb-10">
               ...
               <div><h3 className="text-sm font-semibold text-blue-400 mb-1">{t.sections.conclusion}</h3><div className="text-sm text-slate-300 leading-relaxed"><HtmlContent html={report.conclusion || t.sections.noData} /></div></div>
               {report.references.length > 0 && (<div><h3 className="text-sm font-semibold text-blue-400 mb-1">{t.sections.references}</h3><ul className="list-disc list-inside text-sm text-slate-300 pl-2">{report.references.map((ref, i) => <li key={i}><HtmlContent html={ref} /></li>)}</ul></div>)}
            </div>
